...
void build_icosphere(model_t *model, vec3 pos, float radius, int order) {
    ...
    float dist = sqrtf(phi * phi + 1.0f);

    float p = (phi / dist) * radius;
    float q = (1 / dist) * radius;

    uint32_t indicies[n_vertices];
    indicies[0]  = add_vertex(model, (vec3){ q,  p,  0});
    indicies[1]  = add_vertex(model, (vec3){-q,  p,  0});

    indicies[2]  = add_vertex(model, (vec3){ 0,  q,  p});
    indicies[3]  = add_vertex(model, (vec3){ 0,  q, -p});

    indicies[4]  = add_vertex(model, (vec3){ p,  0,  q});
    indicies[5]  = add_vertex(model, (vec3){ p,  0, -q});
    indicies[6]  = add_vertex(model, (vec3){-p,  0, -q});
    indicies[7]  = add_vertex(model, (vec3){-p,  0,  q});
    
    indicies[8]  = add_vertex(model, (vec3){ 0, -q,  p});
    indicies[9]  = add_vertex(model, (vec3){ 0, -q, -p});
    
    indicies[10] = add_vertex(model, (vec3){ q, -p,  0});
    indicies[11] = add_vertex(model, (vec3){-q, -p,  0});

    uint32_t faces[n_faces];
    uint32_t face_indices[3];

    face_indices[0] = 0; face_indices[1] = 1; face_indices[2] = 2;
    faces[0] = add_face(model, face_indices, 3);

    face_indices[0] = 1; face_indices[1] = 0; face_indices[2] = 3;
    faces[1] = add_face(model, face_indices, 3);

    face_indices[0] = 0; face_indices[1] = 2; face_indices[2] = 4;
    faces[2] = add_face(model, face_indices, 3);

    face_indices[0] = 0; face_indices[1] = 4; face_indices[2] = 5;
    faces[3] = add_face(model, face_indices, 3);

    face_indices[0] = 0; face_indices[1] = 5; face_indices[2] = 3;
    faces[4] = add_face(model, face_indices, 3);

    face_indices[0] = 1; face_indices[1] = 3; face_indices[2] = 6;
    faces[5] = add_face(model, face_indices, 3);

    face_indices[0] = 1; face_indices[1] = 6; face_indices[2] = 7;
    faces[6] = add_face(model, face_indices, 3);

    face_indices[0] = 1; face_indices[1] = 7; face_indices[2] = 2;
    faces[7] = add_face(model, face_indices, 3);

    face_indices[0] = 2; face_indices[1] = 8; face_indices[2] = 4;
    faces[8] = add_face(model, face_indices, 3);
    
    face_indices[0] = 2; face_indices[1] = 7; face_indices[2] = 8;
    faces[9] = add_face(model, face_indices, 3);

    face_indices[0] = 3; face_indices[1] = 5; face_indices[2] = 9;
    faces[10] = add_face(model, face_indices, 3);

    face_indices[0] = 3; face_indices[1] = 9; face_indices[2] = 6;
    faces[11] = add_face(model, face_indices, 3);

    face_indices[0] = 4; face_indices[1] = 8; face_indices[2] = 10;
    faces[12] = add_face(model, face_indices, 3);

    face_indices[0] = 5; face_indices[1] = 10; face_indices[2] = 9;
    faces[13] = add_face(model, face_indices, 3);

    face_indices[0] = 5; face_indices[1] = 4; face_indices[2] = 10;
    faces[14] = add_face(model, face_indices, 3);

    face_indices[0] = 6; face_indices[1] = 11; face_indices[2] = 7;
    faces[15] = add_face(model, face_indices, 3);

    face_indices[0] = 6; face_indices[1] = 9; face_indices[2] = 11;
    faces[16] = add_face(model, face_indices, 3);

    face_indices[0] = 7; face_indices[1] = 11; face_indices[2] = 8;
    faces[17] = add_face(model, face_indices, 3);

    face_indices[0] = 8; face_indices[1] = 11; face_indices[2] = 10;
    faces[18] = add_face(model, face_indices, 3);

    face_indices[0] = 9; face_indices[1] = 10; face_indices[2] = 11;
